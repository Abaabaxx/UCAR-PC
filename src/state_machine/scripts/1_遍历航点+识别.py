...
class RobotStateMachine(object):
    def __init__(self):
        ...
        # 初始化检测相关变量
        self.room_c_objects = None
        self.room_b_objects = None
        self.room_a_objects = None
    ...
    def start_detection_phase(self):
        """开始检测阶段，清空缓冲区并启动定时器"""
        self.detected_objects_buffer = []
        rospy.loginfo("开始检测，等待1秒收集YOLO检测结果...")
    ...
    def finish_detection_phase(self, timer_event):
        """检测阶段结束，处理检测结果并触发相应事件"""
        # 根据当前状态存储检测结果
        if self.current_state == RobotState.DETECT_AT_C1 or self.current_state == RobotState.DETECT_AT_C2:
            self.room_c_objects = list(self.detected_objects_buffer)
            room_name = "C房间"
        elif self.current_state == RobotState.DETECT_AT_B1:
            self.room_b_objects = list(self.detected_objects_buffer)
            room_name = "B房间"
        elif self.current_state == RobotState.DETECT_AT_A1:
            self.room_a_objects = list(self.detected_objects_buffer)
            room_name = "A房间"
        else:
            room_name = "未知房间"
        ...
        # 检查是否检测到物体，并触发相应事件
        if self.detected_objects_buffer:
            objects_info = ", ".join([box.Class for box in self.detected_objects_buffer])
            rospy.loginfo("在%s检测到 %d 个物体: %s", room_name, len(self.detected_objects_buffer), objects_info)
            self.handle_event(Event.DETECT_DONE_FOUND)
        else:
            rospy.loginfo("在%s未检测到任何物体", room_name)
