...
# 该版本如果机器人在C1点要找的是“甜品”(desserts)，但看到了一个高置信度的“苹果”(apple)，它依然会直接前往B1点。
# 但是他是服务调用，是硬编码的
import rospy
import actionlib
import threading
...
class RobotStateMachine(object):
    ...
    def transition(self, new_state):
        ...
        if new_state == RobotState.COMPLETED:
            rospy.loginfo("任务流程已完成，详细结果请查看上方日志。")
    ...
    def start_callback(self, req):
        ...
        with self.state_lock:
            # 在这里硬编码你想要测试的任务类型
            self.current_task_type = 'desserts' 
        
        rospy.loginfo("已设置默认测试任务: %s", self.current_task_type)
        ...
        return TriggerResponse(
            success=True,
            message="State machine started successfully for task: %s" % self.current_task_type
        )
