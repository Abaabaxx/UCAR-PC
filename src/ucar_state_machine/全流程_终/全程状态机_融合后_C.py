...
class Event(object):
    ...
    NODES_SHUTDOWN_COMPLETE = 19
    NODES_SHUTDOWN_TIMEOUT = 20
    LINE_FOLLOWING_DONE = 21
...
class UnifiedStateMachine(object):
    ...
    def execute_state_actions(self):
        ...
        # --- 后半段状态动作 ---
            ...
            if self.final_lane_choice == 'lane1':
                ...
                rospy.logerr("无法确定最终车道选择，无法启动巡线脚本。进入错误状态。")
                self.transition(RobotState.ERROR)
                return
    ...
    def handle_event(self, event):
        ...
        # --- 后半段事件处理 ---
            ...
            if event == Event.NODES_SHUTDOWN_COMPLETE:  # 使用新的事件
                rospy.loginfo("所有节点已成功关闭，准备重启摄像头")
                self.transition(RobotState.RELAUNCH_CAMERA_FOR_LINE_FOLLOWING)
            elif event == Event.NODES_SHUTDOWN_TIMEOUT:  # 使用新的事件
                rospy.logerr("节点关闭超时")
                self.transition(RobotState.ERROR)
