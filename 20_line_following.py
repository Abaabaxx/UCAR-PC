...
class Event(object):
    ...
    YOLO_SHUTDOWN_COMPLETE = 13             # 新增：YOLO节点成功关闭事件
    YOLO_SHUTDOWN_TIMEOUT = 14              # 新增：YOLO节点关闭超时事件
...
class RobotStateMachine(object):
   ...
   # 修正后的 check_for_target_goods 方法
    ...
    def _check_yolo_node_shutdown_status(self, event):
        """检查/darknet_ros节点是否已关闭"""
        try:
            # 获取当前所有节点名称
            node_names = rosnode.get_node_names()
            
            # 检查/darknet_ros节点是否已不存在
            if '/darknet_ros' not in node_names:
                rospy.loginfo("确认/darknet_ros节点已成功关闭")
                
                # 停止相关定时器
                if self.yolo_shutdown_check_timer is not None:
                    self.yolo_shutdown_check_timer.shutdown()
                    self.yolo_shutdown_check_timer = None
                
                if self.yolo_shutdown_timeout_timer is not None:
                ...
                rospy.loginfo("/darknet_ros节点仍在运行，继续等待...")
                # 不做任何操作，等待下一次检查
        except Exception as e:
            rospy.logerr("检查YOLO节点状态时出错: %s", str(e))
            # 停止定时器，避免持续出错
            if self.yolo_shutdown_check_timer is not None:
                self.yolo_shutdown_check_timer.shutdown()
                self.yolo_shutdown_check_timer = None
    ...
    def _handle_yolo_node_shutdown_timeout(self, event):
        """处理YOLO节点关闭超时情况"""
        rospy.logerr("关闭/darknet_ros节点超时")
    ...
    def execute_state_actions(self):
        if self.current_state == RobotState.IDLE:
            ...
            rospy.loginfo("正在尝试关闭/darknet_ros节点...")
            # 发送关闭命令
